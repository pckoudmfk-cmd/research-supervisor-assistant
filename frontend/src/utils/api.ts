...
function ktpAnalysisPrompt(text: string, workType: string, level: string, difficulty: string, direction: string, subjectArea: string) {
  ...
  return `Ты — опытный научный руководитель. Отвечай ТОЛЬКО на русском языке — никакого английского текста в значениях JSON.

Проанализируй учебный материал и предложи научно значимые исследовательские ракурсы.
...
Верни ответ строго в формате JSON (все значения на русском языке):
{
  "topics": [
    {
      "title": "Академическая формулировка темы на русском",
      "angle": "В чём исследовательский ракурс — какую проблему решаем, что изучаем (1–2 предложения на русском)",
      "why": "Почему тема перспективна и актуальна именно сейчас (1 предложение на русском)"
    }
  ]
}`;
}
...
Разработай полноценное научное обоснование темы.
...
{
  "topic": "Уточнённая академическая формулировка темы на русском",
  "object": "Объект исследования — широкая область (на русском)",
  "subject": "Предмет исследования — конкретный аспект (на русском)",
  "relevance": "Актуальность: 2–3 предложения на русском о современных проблемах и противоречиях.",
  "novelty": "Научная новизна: 1–2 предложения на русском о том, что нового вносит данная работа.",
  "hypothesis": "Гипотеза исследования: одно предложение на русском"
}`;
}
...
function planPrompt(topic: string, object: string, subject: string, workType: string, level: string, difficulty: string) {
  ...
  const lv = LEVEL_INFO[level] ?? LEVEL_INFO.vuz;
  const df = DIFFICULTY_INFO[difficulty] ?? DIFFICULTY_INFO.standard;
  return `Ты — научный руководитель. Отвечай ТОЛЬКО на русском языке — никакого английского текста в значениях JSON.

Составь подробный план работы как инструкцию для студента.
...
Верни строго в формате JSON (все значения на русском языке):
{
  "goal": "Цель работы на русском — одно предложение, начинается с глагола (Исследовать / Разработать / Проанализировать...)",
  "objectives": [
    "Задача 1 на русском — конкретное действие",
    "Задача 2",
    "Задача 3",
    "Задача 4",
    "Задача 5"
  ],
  "keywords": ["ключевое слово на русском", "ключевое слово 2"],
  "chapters": [
    {
      "number": 1,
      "title": "Название раздела на русском",
      "description": "Что делать студенту в этом разделе на русском (1–2 предложения)"
    }
  ],
  "methods": ["метод на русском 1", "метод 2", "метод 3"],
  "expectedResult": "Ожидаемый результат на русском"
}

Задач должно быть 5–7. Глав — согласно структуре ${wi.name}. Методов — 3–5.`;
}
